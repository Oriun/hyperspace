window.onload = function () {
  ...
  const demo = new DemoController(hyperspace);
};
...
class HyperSpace {
  ...
  aStarIsBorn(std = 0.2) {
    const star = new Star({
      x: randomInNormalDistribution(std) * this.width,
      y: randomInNormalDistribution(std) * this.height,
      radius: Math.max(
        4,
        Math.min(
          Math.floor(
  ...
  drawDevMode() {
    ...
    ctx.fontWeight = "bold";
    ctx.fillText(`Use arrow keys to modify speed and count`, 10, 64);
    Object.entries({
      speedFactor: this.speedFactor,
      ...
      fps: Math.round(1000 / (Date.now() - this.lastDraw))
  ...
  loop() {
    ...
    this.stars.forEach((star) => {
      ...
      const steps = 5 * this.options.speedFactor;
    ...
    setTimeout(
      () => this.loop(),
      HyperSpace.plannedDuration - Date.now() + this.lastDraw
    );
...
class Star {
  ...
  checkBounds() {
    if (
      ...
      this.x > this.hyper.width + this.hyper.starMaxRadius ||
      this.y < -this.hyper.starMaxRadius ||
      this.y > this.hyper.height + this.hyper.starMaxRadius
    ) {
      this.lastPos.pop();
